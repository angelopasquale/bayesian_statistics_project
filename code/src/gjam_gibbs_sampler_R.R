gjam_gibbs_sampler_R<-function(alpha0,ndraws,burnin,N_stick,r,S,n_sites,x,Y){
  ...
  for(i in 1:ndraws){
    ...
    step1resultr<-STEP1(N_stick,r,sigmaeps2,Z,Q,k,x,Dz,W,V,B,Cardinality_S)
    A<-step1resultr$A
    timer1<-step1resultr$timer+timer1
    
    ######################### STEP 2 ##########################
    ####################### posterior of W ####################
    
    
    step2resultr<-STEP2(sigmaeps2,A,r,V,B,x,n_sites,W)
    W<-step2resultr$W
    timer2<-timer2+step2resultr$timer
    ...
    step3resultr<-STEP3(S,N_stick,sigmaeps2,p,pl,logpl,V,B,W,Z,k)
    k<-step3resultr$k
    timer3<-timer3+step3resultr$timer
    pl<-step3resultr$pl
    logpl<-step3resultr$logpl
    ...
    step4resultr<-STEP4(p,N_stick,S,alpha0,k)
    p<-step4resultr$p
    timer4<-step4resultr$timer4+timer4
    ...
    step5resultr<-STEP5(n_sites,V,B,x,A,W,S,nu,G)
    sigmaeps2<-step5resultr$sigmaeps2
    timer5<-timer5+step5resultr$timer
    ...
    step6resultr<-STEP6(r,Dz,Z,N_stick)
    Dz<-step6resultr$Dz
    timer6<-timer6+step6resultr$timer
    ...
    step7resultr<-STEP7(A,sigmaeps2,S,B,n_sites,W,Y,x)
    V<-step7resultr$V
    V_star<-step7resultr$V_star
    B_star=step7resultr$B_star
    D=step7resultr$D
    timer7<-step7resultr$timer+timer7
    ...
    step8resultr<-STEP8(S,x,sigmaeps2,V_star,W,A,sigmaB,n_cov,D,B_star)
    B<-step8resultr$B
    timer8<-timer8+step8resultr$timer
  ...
  return(list("time_list"=time_list,"chain"=chain))
}
