...
GJAM_Gibbs_Sampler_Rcpp_last <- function(x, Y, r, N_stick, alpha0, posteriorDraws, burnInIterations){
  tic("Data generation:")
  # INPUT PARAMETERS
  ...
  # Q <- matrix(data = 0, nrow = S, ncol = N_stick) if we do in our fashion
  ...
  ##### Step 3 #####
  p = rep(1/N_stick, times = N_stick)
  
  k = c() # each label belongs to {1,...,N_stick}
  ...
  # cardinality_S = rep ( 0, times=N_stick ) if we do in our fashion
