...
for(i in 1:niter){
  ...
  A<-STEP1RCPP(N_stick,r,sigmaeps2,Z,Q,k,x,Dz,W,B,eta_h,Y)
  
  #2
  #W<-STEP2(sigmaeps2,A,r,V,B,x,n_sites,W)
  W<-STEP2RCPP(sigmaeps2,A,r,Y,B,x,n_sites,W)
  #3
  #STEP3(S,N_stick,sigmaeps2,p,pl,logpl)
  k<-STEP3RCPP(p,Y,Z,x,B,W,sigmaeps2,N_stick)
  #if(i==19){k_1<-k}
  #4
 ...
 sigmaeps2<-STEP5RCPP(n_sites,B,x,A,W,S,nu,G,sigmaeps2,Z,k,r,Y)
  #6
...
#D=lista$D
  #8
 B<-STEP8(S,x,sigmaeps2,Y,W,A,sigmaB,n_cov,D)
...
#apply(simplify2array(A),1:2, mean)
#con<-list("A_true"=data$A_true,"A_media"=A_media)
con<-data$A_true
return(con)
}
